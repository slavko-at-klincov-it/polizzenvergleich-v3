...
function writeFeC12AbsenceCategory(run, { postLossCost = false } = {}) {
  ...
  fs.writeFileSync(
    path.join(categoryDirectory, "worksheet.private.json"),
    JSON.stringify({
      catalog: { id: "fe-occurrence-full-draft-v0.9", categoryView },
...
function writeVs22AbsenceCategory(run, { nonTargetWaste = false } = {}) {
  ...
  fs.writeFileSync(
    path.join(categoryDirectory, "worksheet.private.json"),
    JSON.stringify({
      catalog: { id: "vs-occurrence-full-draft-v0.16", categoryView },
...
describe("policy comparison result builder", () => {
  ...
  test("preserves only a valid selected FE-C07 condition-absence audit in the comparison atom", () => {
    ...
    const worksheet = {
      catalog: { id: "fe-occurrence-full-draft-v0.9" },
      requirements: [
        {
          id: "FE-C07",
          requestedFields: ["limit"],
  ...
  test("builds the five-category customer profile with document-level provenance", async () => {
    ...
    expect(result.productProfile).toMatchObject({
      id: "CUSTOMER_CORE_5_V107_VS22_SOURCE_BOUND_CONTINUATION",
      comparisonContractId:
        "PACKAGE_FIRST_QUALIFIED_INCLUSION_ABSENCE_LW20_EQUALITY_FIRE_DEFINITION_VS15_QUALIFIER_VS08_CONSENSUS_OBJECT_FAMILY_ANY_IDENTITY_AMOUNT_LOCAL_CONDITION_VS21_COST_ROLE_BINDING_GROUP_FIELDS_LIMIT_PORTFOLIO_REVIEW_GATE_VS22_LOCAL_WASTE_SCOPE_EXACT_CLAUSE_CODE_FIELD_GOVERNOR_HAZARDOUS_WASTE_PORTFOLIO_HARDENED_VS24_OPTIONAL_LOCAL_LIMIT_EXACT_SCOPE_IDENTITY_GLASS_SCAFFOLDING_COST_EQUALITY_CUSTOMER_REPLAY_VALIDATION_PROOF_LIMIT_LANGUAGE_GATE_VS25_SUM_EQUALIZATION_PRECISION_COMBINED_SCOPE_HEADING_PRECISION_AMOUNT_RECONCILIATION_RELATIVE_LIMIT_PORTFOLIO_TYPED_LIMIT_BASIS_CUSTOMER_REPLAY_SOURCE_BINDING_SUM_EQUALIZATION_TERMINAL_LOCAL_BASIS_BINDING_SOURCE_PROOF_PERCENT_DOCUMENT_BASIS_VS36_SYMBOLIC_LIMITS_EXACT_EVENT_LIMIT_LIST_ITEM_FE_A05_NESTED_LIST_CONTINUATION_PROOF_SOURCE_BOUND_OBJECT_SCOPE_EVIDENCE_INTERNAL_SCOPE_PROVENANCE_SELECTED_SCOPE_REPLAY_FE_C02_CONDITION_SCOPE_DECISION_QUALIFICATION_REPLAY_VS08_WORKSHEET_TRUST_ANCHOR_FE_A01_FE_C07_SOURCE_BOUND_QUALIFICATION_REPLAY_FE_A09_REQUIRED_OBJECT_SCOPE_EL13_OBJECT_SCOPE_IDENTITY_NARROW_ALIAS_SCOPE_IDENTITY_SOURCE_BOUND_TRIAGE_STRUCTURAL_CONCEPT_CONTEXT_VS22_SOURCE_BOUND_CONTINUATION_V68",
  ...
  test("materializes only generally scoped FE-C02 condition-formula targets", () => {
    ...
    expect(atom.membershipConditionScopeComparisonContract).toMatchObject({
