...
describe("LF_REFERENCE_A_DRIVEN_V2 source and semantic contracts", () => {
  ...
  test("uses a focused full-boundary repair after repeated source-bound atomization failures", async () => {
      ...
      requirements: [
        {
          ...
          components: [
            {
              type: "FACT_ROLE",
              label: unit.source.blocks.at(-1).exactText,
              sourceBlockIds: [unit.source.blockIds.at(-1)],
            },
          ],
        },
      ],
      ...
      chat: {
        completions: {
          create: jest.fn(async ({ messages }) => {
            ...
            const response = repairPrompts.length < 3 ? invalid : valid;
            return {
              model: "qwen/qwen3.6-35b-a3b",
              choices: [{ message: { content: JSON.stringify([response]) } }],
              usage: {},
    ...
    expect(result.validation.passed).toBe(true);
    expect(repairPrompts[1]).not.toContain(
      "FOKUSREPARATUR_SOURCE_BOUND_ATOMIZATION"
    );
