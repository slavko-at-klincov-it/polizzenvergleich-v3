...
function validateCatalog(rawCatalog) {
  ...
  if (
    !Number.isInteger(sourceProduct?.physicalPages) ||
    sourceProduct.physicalPages < 1
  )
    throw pilotError("LF_REFERENCE_SOURCE_PAGES_INVALID");
  if (
    !Array.isArray(rawCatalog.categories) ||
    rawCatalog.categories.length === 0
  )
  ...
  const categories = rawCatalog.categories.map((category) => {
    const categoryId = requiredString(
      category?.id,
      "LF_REFERENCE_CATEGORY_ID_REQUIRED"
    );
    ...
    return {
      ...
      label: requiredString(
        category.label,
        "LF_REFERENCE_CATEGORY_LABEL_REQUIRED"
      ),
      requirements: category.requirements.map((requirement) => {
        const id = requiredString(
          requirement?.id,
          "LF_REFERENCE_REQUIREMENT_ID_REQUIRED"
        );
        if (requirementIds.has(id))
          throw pilotError("LF_REFERENCE_REQUIREMENT_ID_DUPLICATE", id);
        requirementIds.add(id);
        if (
          !Number.isInteger(requirement.reference?.page) ||
          requirement.reference.page < 1
        )
        ...
        if (
          !Array.isArray(requirement.aliases) ||
        ...
        return {
          ...
          label: requiredString(
            requirement.label,
            "LF_REFERENCE_REQUIREMENT_LABEL_REQUIRED"
          ),
          factRole: requiredString(
            requirement.factRole,
            "LF_REFERENCE_FACT_ROLE_REQUIRED"
          ),
          reference: {
            page: requirement.reference.page,
            needle: requiredString(
              requirement.reference.needle,
              "LF_REFERENCE_NEEDLE_REQUIRED"
            ),
          },
          query: requiredString(
            requirement.query,
            "LF_REFERENCE_QUERY_REQUIRED"
          ),
  ...
  if (
    !categories.some(({ requirements }) =>
      requirements.some(({ pilot }) => pilot)
    )
  )
  ...
  return {
    ...
    contractId: requiredString(
      rawCatalog.contractId,
      "LF_REFERENCE_CONTRACT_ID_REQUIRED"
    ),
    qaOnly: true,
    sourceProduct: {
      productId: requiredString(
        sourceProduct.productId,
        "LF_REFERENCE_PRODUCT_ID_REQUIRED"
      ),
      version: requiredString(
        sourceProduct.version,
        "LF_REFERENCE_VERSION_REQUIRED"
      ),
    ...
    comparisonPolicy: {
      direction: requiredString(
        rawCatalog.comparisonPolicy.direction,
        "LF_REFERENCE_DIRECTION_REQUIRED"
      ),
      noMatchMeaning: requiredString(
        rawCatalog.comparisonPolicy.noMatchMeaning,
        "LF_REFERENCE_NO_MATCH_MEANING_REQUIRED"
      ),
      missingIsExclusion: false,
      allowMultipleDocuments:
        rawCatalog.comparisonPolicy.allowMultipleDocuments === true,
...
function bindReferenceEvidence(catalog, documentArtifact) {
  ...
  for (const category of catalog.categories)
    for (const requirement of category.requirements) {
      ...
      if (
        !text ||
        !normalize(text).includes(normalize(requirement.reference.needle))
      )
...
function chunksFromArtifacts(documents, artifactsByUuid) {
  return documents.flatMap((document) => {
    ...
    if (
      ...
      throw pilotError(
        "LF_REFERENCE_COUNTERPART_ARTIFACT_INVALID",
        document.uuid
      );
...
function lexicalScore(requirement, candidate) {
  ...
  for (const alias of requirement.aliases) {
    ...
    if (haystack.includes(needle))
      score += Math.max(2, needle.split(" ").length * 2);
    else {
...
function selectCandidates({
  ...
  const lexical = ranked
    ...
    .sort(
      (a, b) =>
        b.lexicalScore - a.lexicalScore || b.semanticScore - a.semanticScore
    )
    .slice(0, topK);
...
function jsonFromModelText(value) {
  ...
  if (start < 0 || end < start)
    throw pilotError("LF_REFERENCE_MODEL_JSON_MISSING");
  try {
...
function validateModelResults(
  ...
  return rawResults.map((result) => {
    const requirementId = requiredString(
      ...
      "LF_REFERENCE_MODEL_REQUIREMENT_ID_REQUIRED"
    );
    if (!expected.has(requirementId) || seen.has(requirementId))
      throw pilotError(
        "LF_REFERENCE_MODEL_REQUIREMENT_ID_INVALID",
        requirementId
      );
    ...
    if (!Array.isArray(result.candidateIds))
      throw pilotError(
        "LF_REFERENCE_MODEL_CANDIDATE_IDS_INVALID",
        requirementId
      );
    const allowed = new Set(
      (candidatesByRequirement.get(requirementId) || []).map(
    ...
    if (candidateIds.some((candidateId) => !allowed.has(candidateId)))
      throw pilotError(
        "LF_REFERENCE_MODEL_CANDIDATE_ID_UNKNOWN",
        requirementId
      );
    if (
      new Set([
        COUNTERPART_STATUS.DIRECT,
        COUNTERPART_STATUS.PARTIAL,
        COUNTERPART_STATUS.RELATED,
      ]).has(result.status) &&
      candidateIds.length === 0
    )
      throw pilotError(
        "LF_REFERENCE_MODEL_MATCH_WITHOUT_SOURCE",
        requirementId
      );
    ...
    return {
      ...
      candidateIds,
      matchSummary: requiredString(
        result.matchSummary,
        "LF_REFERENCE_MODEL_SUMMARY_REQUIRED"
      ),
      unresolved:
        typeof result.unresolved === "string" ? result.unresolved.trim() : "",
    };
  });
}
