...
const PROMPT_CONTRACT_ID = "LF_A_DRIVEN_REQUIREMENT_DECISION_PROMPT_V3";
  ...
  const messages = [
    {
      role: "system",
      content:
        "Du prüfst dynamisch aus Referenzpaket A ermittelte fachliche Anforderungen gegen servergebundene Originalklauseln aus allen Dokumenten B. Antworte ausschließlich als JSON-Array mit exakt einem Objekt je expectedRequirementId und keiner anderen ID. Erfinde keine Texte, Seiten, IDs oder Tatsachen und nutze ausschließlich die vorgelegten candidateIds. GEFUNDEN bedeutet: In B existiert im passenden Kontext ein quellengebundenes Gegenstück zum selben fachlichen Element. Abweichende Werte, Limits, Bedingungen, Umfänge oder ein ausdrücklicher Ausschluss bleiben ein Gegenstück und werden als Abweichung beziehungsweise OPPOSITE ausgewiesen. Eine bloße Keyword-Nennung, Überschrift oder nur verwandte Deckung ist RELATED_ONLY und kein Gegenstück. Ein positives Kernergebnis braucht eine direkt ausgesprochene oder sprachlogisch zwingende Verbindung von Gegenstand, Faktrolle und vertraglicher Wirkung. Ist eine Rechtsfolge in A an einen konkreten Gegenstand, Vorgang oder Auslöser gebunden, muss B denselben nennen oder sprachlogisch zwingend umfassen. Eine Freistellung von einer Pflicht für einen anderen Vorgang bleibt RELATED_ONLY, auch wenn Rechtsfolge und Oberthema ähnlich sind. Konstruiere aus mehreren nur benachbarten Klauseln keine ungeschriebene Ausnahme, Auffangdeckung, Rangfolge, Freistellung oder sonstige Rechtsfolge und unterstelle keine Branchenüblichkeit. Fehlt diese Verbindung, verwende RELATED_ONLY oder NOT_ESTABLISHED. Ein ausdrücklich umfassender Oberbegriff darf ein benanntes Unterelement als Gegenstück umfassen, wenn Kontext und Faktrolle gleich sind; ein engerer oder abweichender Umfang wird separat ausgewiesen. Eine funktionsgleiche vertragliche Wirkung darf trotz anderem Wortlaut oder Mechanismus denselben Kern bilden, aber ein nur ähnlicher wirtschaftlicher Zweck genügt nicht. Prüfe jede Komponente einzeln und zitiere nur die kleinste Menge von candidateIds, die die jeweilige Feststellung tatsächlich trägt. outcome ist MATCH, COUNTERPART_WITH_DIFFERENCE, OPPOSITE, RELATED_ONLY oder NOT_ESTABLISHED. COUNTERPART_WITH_DIFFERENCE ist nur für SCOPE, CONDITION, VALUE_AND_UNIT, LIMIT_BASIS, DEDUCTIBLE oder TEMPORAL_VALIDITY zulässig. NOT_ESTABLISHED hat candidateIds exakt []; alle anderen Outcomes benötigen mindestens eine candidateId. contextFinding beschreibt, ob die Quellen dasselbe fachliche Element im passenden Objekt-/Gefahr-/Schaden-/Rollen-Kontext behandeln. unmodeledDifferences enthält nur zusätzliche belegte Unterschiede als {dimension,description,candidateIds}. Ausgabe je Anforderung exakt: {requirementId,contextFinding:{outcome,candidateIds},componentFindings:[{componentId,dimension,outcome,candidateIds}],unmodeledDifferences:[...],rationale}. Jede vorgelegte Komponente muss exakt einmal vorkommen.",
    },
    {
      role: "user",
