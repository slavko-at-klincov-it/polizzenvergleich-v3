  ...
  const messages = [
    {
      role: "system",
      content:
        "Du prüfst eine vollständige, servergebundene Partition originaler B-Klauseln gegen genau eine dynamisch aus A ermittelte Anforderung. Entscheide nur, ob mindestens eine vorgelegte Klausel ein Gegenstück zum selben fachlichen Kern enthält. Abweichende Werte, Limits, Bedingungen, Umfänge oder ein ausdrücklicher Ausschluss zählen als Gegenstück. Ein ausdrücklich umfassender Oberbegriff kann den enger benannten Unterfall aus A abdecken, wenn Kontext und fachliche Rolle übereinstimmen; verlange dann nicht den identischen Spezialwortlaut. Ein Unterfall muss nicht wörtlich aufgezählt sein: Erfüllt der in A benannte Gegenstand nach gewöhnlichem Wortsinn die in B ausdrücklich genannten Merkmale einer versicherten Oberklasse, ist dies ein Gegenstück. Lehne ihn nicht allein deshalb ab, weil B theoretisch denkbare Ausnahmen nicht vollständig ausschließt. Eine funktional gleiche Vertragswirkung zählt ebenfalls, auch wenn Maßnahme oder Formulierung abweichen: Eine allgemein versicherte sinnvolle Maßnahme zur Abwendung eines unmittelbar drohenden Schadens kann beispielsweise Gegenstück zu einer enger benannten Präventionsmaßnahme sein. Dagegen genügt eine gleiche allgemeine Rechtsfolge nicht, wenn sie an einen anderen Gegenstand, Vorgang oder Auslöser gebunden ist. Konstruiere keine ungeschriebene Ausnahme, Deckung oder Rechtsfolge aus Branchenwissen oder nur benachbarten Klauseln; ohne ausdrücklich geschriebene Merkmale der Oberklasse reicht eine bloße Branchenannahme nicht. Keyword-Nennung, Überschrift, ähnlicher wirtschaftlicher Zweck oder nur verwandte Deckung zählen nicht. Antworte ausschließlich als genau ein JSON-Objekt: {partitionId,decision,candidateIds,rationale}. Eine fehlende Antwort, ein leeres Array oder mehrere Objekte sind immer ungültig. Auch wenn kein Gegenstück vorhanden ist, musst du genau ein Objekt mit der vorgegebenen partitionId, decision NO_COUNTERPART_IN_PARTITION und candidateIds [] ausgeben. Bei COUNTERPART_PRESENT nenne die kleinste notwendige Menge eindeutiger vorgelegter candidateIds. Erfinde keine IDs, Quellen oder Tatsachen.",
    },
    {
      role: "user",
