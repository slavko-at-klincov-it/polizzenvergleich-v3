    ...
    try {
      ...
      messages = [
        ...
        { role: "assistant", content: rawText },
        {
          ...
          )}. Korrigiere ausschließlich das JSON-Array. Verwende exakt alle expectedUnitIds einmal und keine unbekannten IDs. primaryClass muss in semanticClasses enthalten sein. Kopiere displayLabel und alle Komponentenfelder exakt aus den referenzierten sourceBlocks; formuliere keine Kurzlabels. Jede Komponente braucht ein nichtleeres label. COVERAGE_EFFECT ist eine eigene Komponente mit nichtleerem wörtlichem Wirkungs-label und coverageEffect. Vertragsrollen sind DEFINITION/FACT_ROLE, keine versicherten Objekte. Administrative Pflichten oder Voraussetzungen sind CONDITION oder OBLIGATION als semanticClass, ihre Komponente hat aber immer type CONDITION. Terminalklassen wie VARIANT, OBLIGATION, LIMIT, COST und DURATION sind niemals component.type. LIMIT benötigt VALUE_AND_UNIT mit rawValue; VARIANT verwendet SCOPE.`,
        },
      ];
    } catch (error) {
      attempts.push({
